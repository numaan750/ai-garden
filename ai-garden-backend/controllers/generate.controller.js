...
export const generateImage = async (req, res) => {
  try {
    ...
    // ✅ FIX: declare updatedUser OUTSIDE the if block
    let updatedUser = null;

    if (!user.isPremium) {
      updatedUser = await loginSchema.findOneAndUpdate(
    ...
    // ✅ Now updatedUser is accessible here
    res.status(200).json({
      status: "success",
      imageUrl,
      creditsLeft: user.isPremium
        ? "unlimited"
        : updatedUser
          ? updatedUser.credits
          : 0,
