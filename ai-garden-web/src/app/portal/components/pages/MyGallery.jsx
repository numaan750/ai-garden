    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col gap-4 pb-4">
        <div className="flex items-center gap-2">
          <button onClick={onBack} className="text-[#ABD8FC] cursor-pointer">
        ...
        <div
          className="relative rounded-2xl overflow-hidden bg-[#C4D0BB] border border-[#05C847]"
          style={{ height: "260px" }}
        >
          <img
            src={item.imageUrl}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[#164C1A] text-[14px] font-medium">
          ...
          <textarea
            ...
            className="w-full bg-[#C4D0BB] border border-[#05C847] text-[#164C1A] text-[14px] rounded-2xl p-4 focus:border-[#3B7FFF] outline-none resize-none placeholder:text-[#164C1A]/40"
            rows={3}
          />
        </div>
      </div>
          ...
          className={`w-full max-w-[500px] py-3 rounded-full font-semibold text-[18px] text-[#F3F3F3] transition flex items-center justify-center gap-2
            ${
              editPrompt.trim() && !loading
                ? "bg-gradient-to-b from-[#34B23D] to-[#164C1A] text-[#164C1A] cursor-pointer hover:opacity-90"
    ...
    try {
      ...
      if (window.showSaveFilePicker) {
        const fileHandle = await window.showSaveFilePicker({
          suggestedName: `ai-garden-${item.toolId || "result"}.jpg`,
          types: [
            {
              description: "Image",
        ...
        link.href = blobUrl;
        link.download = `ai-garden-${item.toolId || "result"}.jpg`;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto flex flex-col gap-4 pb-4">
        ...
        <div
          ref={containerRef}
          className="relative rounded-xl overflow-hidden bg-[#C4D0BB] border border-[#05C847] h-[280px] sm:h-[320px] md:h-[360px]"
        ...
        {item.toolId && (
          <div className="flex">
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-[#CBE3C0] text-black capitalize">
  ...
  return (
    <div className="text-white">
      <div className="flex gap-3 mb-6">
        <p className="text-[16px] sm:text-[18px] font-bold text-[#164C1A]">
          Recent Activity
        </p>
      </div>
      ...
      {gallery.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
          ...
          <p className="text-[15px] font-medium text-[#696a70]">
            Nothing Here Yet
          </p>
        </div>
      ) : (
