...
const Login = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden flex items-center justify-center bg-white py-5">
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-all duration-0"
        style={{
          backgroundImage: `url(${
            isSignup ? "/images/sign-in-screen.png" : "/images/Log-in-screen.png"
          })`,
        }}
      />{" "}
      <div className="relative min-h-screen flex flex-col lg:flex-row-reverse">
        <div className="w-full flex items-center justify-center px-4">
          <div className="w-full sm:w-[600px] bg-[#2E3A34] border border-[#000000] rounded-2xl p-8 shadow-xl">
            ...
            <h1 className="text-white font-bold text-center text-[22px] sm:text-[24px] lg:text-[26px]">
              {isSignup ? "Create an Account" : "Log in to Ai Garden"}
            </h1>
            <p className="text-white text-[12px] sm:text-[12px] lg:text-[14px] text-center mb-2">
              {isSignup
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {isSignup && (
                <div className="relative w-full">
                  <FaRegUser className="absolute left-4 top-1/2 -translate-y-1/2 text-white text-[16px] sm:text-[18px]" />
                  <input
                    type="text"
                    name="username"
                    ...
                    className="w-full pl-10 sm:pl-12 pr-4 sm:pr-5 py-3 sm:py-4 rounded-xl bg-[#3A463F] border border-[#232B27] text-[#A4A4A4] placeholder-white outline-none text-[14px] sm:text-[16px] font-medium"
                  />
                </div>
              )}

              <div className="relative w-full">
                <MdOutlineMail className="absolute left-4 top-1/2 -translate-y-1/2 text-white text-[20px]" />
                <input
                  type="email"
                  name="email"
                  ...
                  className="w-full pl-12 pr-5 py-4 rounded-xl bg-[#3A463F] border border-[#232B27] text-white placeholder-white outline-none text-[16px] font-medium"
                />
              </div>

              <div className="relative w-full">
                <CiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-white text-[20px]" />
                ...
                <input
                  ...
                  className="w-full pl-12 pr-12 py-4 rounded-xl bg-[#3A463F] border border-[#232B27] text-white placeholder-white outline-none text-[16px] font-medium"
                />
                <button
                  type="button"
              ...
              <div className="flex justify-center ">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full cursor-pointer py-3 rounded-full bg-gradient-to-b from-[#34B23D] to-[#164C1A] text-white font-semibold text-[18px] tracking-wide"
                >
                  {loading
                    ? "Please wait..."
                    : isSignup
              ...
              <div className="flex">
                <button
                  ...
                  disabled={loading}
                  className="flex-1 py-3 cursor-pointer rounded-full bg-[#3E4A44] border border-[#2A332F] text-white flex items-center justify-center gap-3 text-[15px]"
                >
                ...
                {isSignup ? (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={toggleMode}
                      className="text-[#34B23D] underline cursor-pointer"
                    >
                      Login
                    </button>
                  </>
